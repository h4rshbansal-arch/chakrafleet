...
export function AppSidebar({ isSheet = false }: AppSidebarProps) {
  const { user } = useAuth();
  const { t } = useLanguage();
  ...
  const NavLinks = ({ isTooltip = false }: { isTooltip?: boolean }) => (
    <TooltipProvider>
       <nav className={cn("flex flex-col items-center gap-4 px-2 sm:py-5", isSheet && "items-start gap-2 p-0")}>
        {!isSheet && (
           <Link
            href="/dashboard"
          ...
          >
            ...
            <span className="sr-only">ChakraFleet</span>
          </Link>
        )}
        {currentNavItems.map((item, index) => {
          const LinkContent = (
              <>
                  ...
                  {isSheet ? item.label : <span className="sr-only">{item.label}</span>}
              </>
          );

          const linkClassName = isSheet ? "flex items-center gap-4 px-2.5 text-muted-foreground hover:text-foreground" : "flex h-9 w-9 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:text-foreground md:h-8 md:w-8";
